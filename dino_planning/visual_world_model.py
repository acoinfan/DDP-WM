...
class VWorldModel(nn.Module):
    def __init__(
        ...
        log.info(f"num_action_repeat: {self.num_action_repeat}")
        log.info(f"num_proprio_repeat: {self.num_proprio_repeat}")
        log.info(f"proprio encoder: {proprio_encoder}")
        log.info(f"action encoder: {action_encoder}")
        log.info(f"proprio_dim: {proprio_dim}, after repeat: {self.proprio_dim}")
        log.info(f"action_dim: {action_dim}, after repeat: {self.action_dim}")
        log.info(f"emb_dim: {self.emb_dim}")

        self.concat_dim = concat_dim  # 0 or 1
        if concat_dim not in (0, 1):
            raise ValueError(f"concat_dim {concat_dim} not supported (0 or 1)")
        log.info(f"Model emb_dim: {self.emb_dim}")
    ...
    def forward(self, obs, act):
        ...
        if self.predictor is not None:
            ...
            if self.decoder is not None:
                ...
                decoder_loss_pred = recon_loss_pred + self.decoder_latent_loss_weight * diff_pred
            ...
            if self.concat_dim == 0:
                ...
                z_loss = self.emb_criterion(
                    z_pred[:, :, :, : -self.action_dim], z_tgt[:, :, :, : -self.action_dim].detach()
                )
