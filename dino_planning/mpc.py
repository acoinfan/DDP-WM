...
class MPCPlanner(BasePlanner):
    ...
    def plan(self, obs_0, obs_g, actions=None):
        ...
        while not np.all(self.is_success) and self.iter < self.max_iter:
            ...
            t_plan = time.perf_counter() - t_round
            ...
            log.info(f"MPC iter {self.iter} Eval ------- ")
            ...
            t_eval = time.perf_counter() - t_round - t_plan
            new_successes = successes & ~self.is_success  # Identify new successes
            self.is_success = self.is_success | successes  # Update overall success status
            self.action_len[new_successes] = (
                self.iter + 1
            ) * self.n_taken_actions  # Update only for the newly successful trajectories

            log.info(f"self.is_success: {self.is_success}")
            logs = {f"{self.logging_prefix}/{k}": v for k, v in logs.items()}
            logs.update({"step": self.iter + 1})
            log.info(
                f"[MPC] round {self.iter}: plan={t_plan:.1f}s env-eval={t_eval:.1f}s "
                f"round_total={time.perf_counter() - t_round:.1f}s "
                f"success={logs.get(self.logging_prefix + '/success_rate', float('nan')):.3f} "
                f"n_success={int(self.is_success.sum())}/{len(self.is_success)}"
            )
            self.dump_logs(logs)
